        ...
        cells.forEach(cell => {
            ...
            toCheck.forEach(point => {
                ...
                if (!checked.has(str)) {
                    ...
                    if (gridPos.x < 0 || gridPos.y < 0 || grid[gridPos.y][gridPos.x])
                        return;
                    let count = 0;
                    for (let y = -1; y <= 1; y++) {
                        for (let x = -1; x <= 1; x++) {
    ...
    handleMouseDown() {
        ...
        for (let y = -size; y < size; y++) {
            for (let x = -size; x < size; x++) {
                ...
                if (!existingCell)
                    this.createCellAtPoint(newPoint);
            }
        }
    }
    update(dt) {
        ...
        const renderer = this.engine.renderer;
        renderer.rect(gridAlign.x, gridAlign.y, CELL_SIZE, CELL_SIZE, [255, 255, 255, 100]);
